...
\begin{frame}[t]{План доклада}
    ...
    \begin{alertblock}{Основные результаты}
        \begin{itemize}
            \item Ассоциативная точная обертывающая алгебра типа $A_n$, допускающая градуировку,
                  согласованную с градуировкой Картана, единственна с точностью до изоморфизма и
                  изоморфна $NT(n+1,\K)$.
...
\begin{frame}[t]{Неассоциативные точные обертывающие алгебры}
    ...
    \begin{itemize}
        \item А.\,А.~Алберт\sourcefootnote{albert48} называет произвольную алгебру $R$ (не
              обязательно ассоциативную) \term{Ли-допустимой}, когда $R^{(-)}$ есть алгебра
              Ли.
...
\begin{frame}[t]{Определение умножения в $R$ по заданной структуре $R^{(-)}$}
    ...
    \begin{itemize}
        ...
        \item Вопрос об условиях однозначности неассоциативной обертывающей алгебры отмечал
...
\begin{frame}[t]{Нильтреугольные алгебры Шевалле}
    ...
    \begin{itemize}
        ...
        \item Тем самым определяется алгебра Шевалле $\mathfrak{L}(\Phi,\K)$ над любым полем
              $\K$.
        \item Ее подалгебру
              \[
...
\begin{frame}[t]{Тип $A_{n}$ и постановка задачи}
    ...
    \begin{itemize}
        \item Ранее единственность $NT(n,\K)$ внутри семейства $R_\Phi$ исследовалась при
...
\begin{frame}[t]{Тонкие градуировки}
    \begin{itemize}
        \item Будем говорить, что $\Gamma_2$ является \emph{уточнением} градуировки
              $\Gamma_1$, если существует гомоморфизм огрубления $\phi\colon H\to G$ такой,
              что для каждого $h\in H$ выполняется включение $A_h\subset A_{\phi(h)}$.
...
\begin{frame}[t]{Выбор умножения на идеале $T(r_\Phi)$}
    ...
    \begin{itemize}
        \item Для типа $B_n$ используется то, что коммутант алгебры Ли $T(r_{10})$ лежит в
              центре, поэтому
              \[
                  T(r_{10})^3=0.
              \]
...
\begin{frame}[t]{Почему эти конструкции неассоциативны}
    ...
    \begin{block}{Типы $C_n$ и $D_n$}
        Здесь нарушается условие \[\phi_{xy}=\phi_x\phi_y.\] При $x=e_{21}$, $y=e_{32}$ и $u=e_{2,-1}\in T(r_\Phi)$ имеем $\phi_{xy}(u)=0$,
        но $\phi_x\phi_y(u)\ne 0$.
    \end{block}
\end{frame}
