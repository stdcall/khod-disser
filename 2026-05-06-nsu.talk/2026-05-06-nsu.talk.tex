...
\begin{frame}[t]{Выбор умножения на идеале $T(r_\Phi)$}
    ...
    \begin{itemize}
        \item Для типа $B_n$ используется то, что коммутант алгебры Ли $T(r_{10})$ лежит в ее
              центре, поэтому
              \[
                  T(r_{10})^3=0.
              \]
...
\begin{frame}[t]{Почему эти конструкции неассоциативны}
    ...
    \begin{block}{Тип $B_n$}
        ...
        $\quot{T(r_{10})}{Z}$ имеет размерность не менее $2$, а проекция правого
        аннулятора в $\quot{T(r_{10})}{Z}$ --- не более $1$.
    \end{block}
